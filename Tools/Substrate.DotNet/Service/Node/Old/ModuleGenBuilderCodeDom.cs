...
{
   public class ModuleGenBuilderCodeDom : ModuleBuilderBaseCodeDom
   {
      private ModuleGenBuilderCodeDom(string projectName, uint id, PalletModule module, NodeTypeResolver typeDict, Dictionary<uint, NodeType> nodeTypes) :
          base(projectName, id, module, typeDict, nodeTypes)
      {
      }

      public static ModuleGenBuilderCodeDom Init(string projectName, uint id, PalletModule module, NodeTypeResolver typeDict, Dictionary<uint, NodeType> nodeTypes)
      {
         return new ModuleGenBuilderCodeDom(projectName, id, module, typeDict, nodeTypes);
      }

      public override ModuleGenBuilderCodeDom Create()
      {
         ...
         if (storage?.Entries != null)
         {
            foreach (Entry entry in storage.Entries)
            {
               ...
               else if (entry.StorageType == Storage.Type.Map)
               {
                  ...
                  // add storage key mapping in constructor
                  constructor.Statements.Add(ModuleGenBuilderCodeDom.AddPropertyValues(ModuleGenBuilderCodeDom.GetStorageMapString(key.ToString(), value.ToString(), storage.Prefix, entry.Name, hashers), "_client.StorageKeyDict"));
               }
               else
               {
