...
{
   public class StructBuilderCodeDom : TypeBuilderBaseCodeDom
   {
      private StructBuilderCodeDom(string projectName, uint id, NodeTypeComposite typeDef, NodeTypeResolver typeDict)
      ...
      private CodeMemberMethod GetDecode(NodeTypeField[] typeFields)
      {
         ...
         if (typeFields != null)
         {
            for (int i = 0; i < typeFields.Length; i++)
            {
               ...
               string fieldName = StructBuilderCodeDom.GetFieldName(typeField, "value", typeFields.Length, i);
      ...
      public static BuilderBaseCodeDom Init(string projectName, uint id, NodeTypeComposite typeDef, NodeTypeResolver typeDict)
      {
         return new StructBuilderCodeDom(projectName, id, typeDef, typeDict);
      }

      public override TypeBuilderBaseCodeDom Create()
      {
         ...
         if (typeDef.TypeFields != null)
         {
            for (int i = 0; i < typeDef.TypeFields.Length; i++)
            {
               ...
               string fieldName = StructBuilderCodeDom.GetFieldName(typeField, "value", typeDef.TypeFields.Length, i);

               NodeTypeResolved fullItem = GetFullItemPath(typeField.TypeId);

               CodeMemberField field = StructBuilderCodeDom.GetPropertyField(fieldName, fullItem.ToString());

               // add comment to field if exists
               field.Comments.AddRange(GetComments(typeField.Docs, null, fieldName));

               targetClass.Members.Add(field);
               targetClass.Members.Add(StructBuilderCodeDom.GetProperty(fieldName, field));
            }
         }

         CodeMemberMethod encodeMethod = StructBuilderCodeDom.GetEncode(typeDef.TypeFields);
